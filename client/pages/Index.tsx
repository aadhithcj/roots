...
import {
  Search, Leaf, Thermometer, Droplets, Cloud, Info, Loader2, RefreshCw, Zap,
  Navigation, Clock, TrendingUp, BarChart3, Play, X, ChevronRight, Sparkles,
} from "lucide-react";
...
// --- INTERFACES ---
interface SoilData { nitrogen: string; phosphorus: string; potassium: string; ph: string; }
interface CropRecommendation { name: string; confidence: number; description: string; trend: "up" | "down" | "stable"; historicalData: number[]; }
interface LocationSuggestion { name: string; region: string; coordinates: { lat: number; lng: number }; }
...
export default function Index() {
  ...
  const { weatherData, isLoading: isLoadingWeather, error: weatherError, fetchWeatherData } = useWeather();
  const [soilData, setSoilData] = useState<SoilData>({ nitrogen: "", phosphorus: "", potassium: "", ph: "", });
  ...
  // Effects
  useEffect(() => { const handleKeyboard = (e: KeyboardEvent) => { if (e.key === " " && e.ctrlKey) { e.preventDefault(); searchInputRef.current?.focus(); } if (e.key === "Enter" && e.ctrlKey && document.activeElement === searchInputRef.current) { e.preventDefault(); handleSearch(); } if (e.key === "?" && e.ctrlKey) { e.preventDefault(); setShowTutorial(true); } }; window.addEventListener("keydown", handleKeyboard); return () => window.removeEventListener("keydown", handleKeyboard); }, []);
  ...
  // API Logic
  const fetchAndSetWeatherData = async (lat: number, lng: number) => {
    setCurrentStep(1);
  ...
  const recommendCrops = async () => {
    ...
    setCurrentStep(3);
    const payload = { N: parseFloat(soilData.nitrogen), P: parseFloat(soilData.phosphorus), K: parseFloat(soilData.potassium), ph: parseFloat(soilData.ph), temperature: weatherData.temperature, humidity: weatherData.humidity, rainfall: weatherData.rainfall, };
    try {
      const response = await fetch(`${API_BASE_URL}/api/recommend`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload), });
      if (!response.ok) { const errorData = await response.json(); throw new Error(errorData.error || "Failed to get recommendations"); }
      const data = await response.json();
      ...
      <main className="container mx-auto px-6 py-12 space-y-12">
        ...
        <section className="relative">
           <Card className="overflow-hidden shadow-2xl border-border bg-card/50 backdrop-blur-sm rounded-[40px] transform transition-all duration-300 hover:shadow-3xl hover:scale-[1.01]">
            <CardHeader className="bg-agro-green/10 border-b border-agro-green/20">
              ...
              <p className="text-muted-foreground">Search for any location to get started with crop analysis</p>
            </CardHeader>
            <CardContent className="p-8">
              <div className="flex space-x-4 mb-6">
                <div className="flex-1 relative">
                  <Input ref={searchInputRef} placeholder="Search for a location... (Ctrl+Space)" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} onKeyPress={(e) => e.key === "Enter" && handleSearch()} className="h-14 text-lg border-border bg-secondary/50 focus:border-agro-green focus:ring-agro-green/20 focus:ring-4 rounded-3xl transition-all duration-300 focus:shadow-lg focus:shadow-agro-green/20" />
                  {searchQuery && ( <Button variant="ghost" size="sm" onClick={() => setSearchQuery("")} className="absolute right-2 top-1/2 transform -translate-y-1/2 hover:bg-muted/50 rounded-full"> <X className="w-4 h-4" /> </Button> )}
                </div>
              ...
              <div className="mb-4">
                <p className="text-sm font-medium text-foreground mb-3">Popular Farming Regions:</p>
        ...
        {selectedLocation && (
          <section className="animate-in fade-in-50 duration-500">
            {isLoadingWeather ? ( <WeatherSkeleton /> ) : weatherError ? (
              <Card className="border-red-500/20 bg-red-500/10 rounded-[40px]">
                <CardContent className="p-6 flex items-center justify-between">
                  ...
                  <Button onClick={() => selectedLocation && fetchAndSetWeatherData(selectedLocation.lat, selectedLocation.lng)} variant="outline" size="sm" className="border-red-500/30 text-red-400 hover:bg-red-500/20 rounded-3xl">
              ...
              weatherData && (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <Card className="shadow-xl border-border bg-card/50 backdrop-blur-sm hover:shadow-2xl transition-all duration-300 hover:scale-105 rounded-[40px]"> <CardContent className="p-8 text-center"> <div className="w-16 h-16 bg-orange-500/20 rounded-2xl flex items-center justify-center mx-auto mb-4"><AnimatedThermometer /></div> <h3 className="font-semibold text-foreground mb-2 text-lg">Temperature</h3> <p className="text-3xl font-bold text-orange-400">{weatherData.temperature}°C</p> </CardContent> </Card>
                  <Card className="shadow-xl border-border bg-card/50 backdrop-blur-sm hover:shadow-2xl transition-all duration-300 hover:scale-105 rounded-[40px]"> <CardContent className="p-8 text-center"> <div className="w-16 h-16 bg-blue-500/20 rounded-2xl flex items-center justify-center mx-auto mb-4"><AnimatedDroplets /></div> <h3 className="font-semibold text-foreground mb-2 text-lg">Humidity</h3> <p className="text-3xl font-bold text-blue-400">{weatherData.humidity}%</p> </CardContent> </Card>
                  <Card className="shadow-xl border-border bg-card/50 backdrop-blur-sm hover:shadow-2xl transition-all duration-300 hover:scale-105 rounded-[40px]"> <CardContent className="p-8 text-center"> <div className="w-16 h-16 bg-cyan-500/20 rounded-2xl flex items-center justify-center mx-auto mb-4"><AnimatedCloud /></div> <h3 className="font-semibold text-foreground mb-2 text-lg">Rainfall</h3> <p className="text-3xl font-bold text-cyan-400">{weatherData.rainfall}mm</p> </CardContent> </Card>
                </div>
              )
            )}
          </section>
        ...
        {weatherData && (
          <section className="animate-in fade-in-50 duration-700">
            <Card className="shadow-2xl border-border bg-card/50 backdrop-blur-sm rounded-[40px] transform transition-all duration-300 hover:shadow-3xl">
              <CardHeader className="bg-agro-green/10 border-b border-agro-green/20"> <CardTitle className="text-foreground text-xl">Soil Analysis</CardTitle> <p className="text-muted-foreground">Enter your soil nutrient levels for accurate crop recommendations</p> </CardHeader>
              <CardContent className="p-8">
        ...
        {showResults && cropRecommendations.length > 0 && (
          <section className="animate-in fade-in-50 duration-900">
            <Card className="shadow-2xl border-border bg-card/50 backdrop-blur-sm rounded-[40px]">
              <CardHeader className="bg-agro-green/10 border-b border-agro-green/20"> <CardTitle className="text-foreground text-xl">Recommended Crops</CardTitle> <p className="text-muted-foreground">AI-powered recommendations based on your location, weather, and soil conditions</p> </CardHeader>
              <CardContent className="p-8">
